...
class Global_parameter extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'setting/Global_parameter?flag='.$_GET['flag'].'');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('setting/Global_parameter_model', 'Global_parameter');
        /*enable profiler*/
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'setting/Global_parameter/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id.'?flag='.$_GET['flag']);
            /*get value by id*/
            ...
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'setting/Global_parameter/'.strtolower(get_class($this)).'/form');
    ...
    public function show($id)
    {
        ...
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'setting/Global_parameter/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id.'?flag='.$_GET['flag']);
    ...
    public function get_data()
    {
        ...
        foreach ($list as $row_list) {
            ...
            $row[] = '<div style="text-align: center">
                        '.$this->authuser->show_button('setting/Global_parameter?flag='.$_GET['flag'].'','U',$row_list->id,'67/'.$_GET['flag'].'').'
                        '.$this->authuser->show_button('setting/Global_parameter?flag='.$_GET['flag'].'','D',$row_list->id,67).'
                      </div>'; 
