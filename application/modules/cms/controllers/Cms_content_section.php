...
class Cms_content_section extends MX_Controller {
    ...
    function __construct() {
        ...
        parent::__construct();
        /*breadcrumb default*/
        $this->breadcrumbs->push('Index', 'cms/Cms_content_section');
        /*session redirect login if not login*/
        if($this->session->userdata('logged')!=TRUE){
            echo 'Session Expired !'; exit;
        }
        /*load model*/
        $this->load->model('Cms_content_section_model', 'Cms_content_section');
        /*enable profiler*/
    ...
    public function index() { 
        ...
        /*load view index*/
        $this->load->view('Cms_content_section/index', $data);
    }
    ...
    public function form($id='')
    {
        ...
        if( $id != '' ){
            ...
            $this->breadcrumbs->push('Edit '.strtolower($this->title).'', 'Cms_content_section/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
            /*get value by id*/
            $data['value'] = $this->Cms_content_section->get_by_id($id);
            /*initialize flag for form*/
            $data['flag'] = "update";
        }else{
            /*breadcrumbs for create or add row*/
            $this->breadcrumbs->push('Add '.strtolower($this->title).'', 'Cms_content_section/'.strtolower(get_class($this)).'/form');
        ...
        /*load form view*/
        $this->load->view('Cms_content_section/form', $data);
    }
    /*function for view data only*/
    public function show($id)
    {
        /*breadcrumbs for view*/
        $this->breadcrumbs->push('View '.strtolower($this->title).'', 'Cms_content_section/'.strtolower(get_class($this)).'/'.__FUNCTION__.'/'.$id);
        ...
        $data['breadcrumbs'] = $this->breadcrumbs->show();
        /*load form view*/
        $this->load->view('Cms_content_section/form_read', $data);
    }

    public function get_data()
    {
        /*get data from model*/
        $list = $this->Cms_content_section->get_datatables();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Cms_content_section->count_all(),
                        "recordsFiltered" => $this->Cms_content_section->count_filtered(),
                        "data" => $data,
                );
    ...
    //    print_r($_FILES);die;
        ...
        {                       
            ...
            $dataexc = array(
                ...
                'content_owner' => $this->regex->_genRegex('admin', 'RGXQSL'),
                'content_description' => $this->regex->_genRegex($val->set_value('content_description'), 'RGXQSL'),
                'content_publish_date' => $this->regex->_genRegex(date('Y-m-d'), 'RGXQSL'),
                'is_active' => $this->input->post('is_active'),
            );

            // upload attachment
            if(isset($_FILES['content_cover']['name']) AND !empty($_FILES['content_cover']['name'])){
            ...
            if($id==0){
                ...
                /*save post data*/
                $newId = $this->Cms_content_section->save($dataexc);
                /*save logs*/
                $this->logs->save('Cms_content_section', $newId, 'insert new record on '.$this->title.' module', json_encode($dataexc),'content_id');
            }else{
                $dataexc['updated_date'] = date('Y-m-d H:i:s');
                $dataexc['updated_by'] = json_encode(array('user_id' =>$this->regex->_genRegex($this->session->userdata('user')->user_id,'RGXINT'), 'fullname' => $this->regex->_genRegex($this->session->userdata('user')->fullname,'RGXQSL')));
                /*update record*/
                $this->Cms_content_section->update(array('content_id' => $id), $dataexc);
                $newId = $id;
                /*save logs*/
                $this->logs->save('Cms_content_section', $newId, 'update record on '.$this->title.' module', json_encode($dataexc),'content_id');
            }
    ...
    public function delete()
    {
        ...
        $toArray = explode(',',$id);
        if($id!=null){
            if($this->Cms_content_section->delete_by_id($toArray)){
                $this->logs->save('Cms_content_section', $id, 'delete record', '', 'id');
