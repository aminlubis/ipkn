...
class Templates extends MX_Controller {
	...
	public function index($data, $template_name = null)
	{
      ...
      // If there is no template name parameter passed, we just use the default.
		...
		if($this->session->userdata('user')->flag_user == 'ttdi'){
            $this->load->view('templates/content_view_ttdi', $data);
        }elseif ($this->session->userdata('user')->flag_user == 'ipkn') {
            $this->load->view('templates/content_view_ipkn', $data);
        }else{
            $this->load->view('templates/content_view_admin', $data);
        }
	...
	public function getGraphModule(){
	    ...
	    $data = [];
        $data[0] = array(
            'nameid' => 'polar-graph-1',
            'style' => 'polar',
            'col_size' => 12,
            'url' => 'Templates/Templates/graph?prefix=1&TypeChart=polar&style=1&mod=1',
            );
