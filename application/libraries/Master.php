...
final Class Master {
	...
	function custom_selection_with_two_label($custom=array(), $nid='',$name,$id,$class='',$required='',$inline='') {
		...
		<select class="'.$class.'" name="'.$name.'" id="'.$id.'" '.$readonly.' '.$required.' >
			<option value="" '.$selected.'> - Select All - </option>';
				...
				foreach($data as $row){
					$sel = $nid==$row[$custom['id']]?'selected':'';
					$field.='<option value="'.$row[$custom['id']].'" '.$sel.' >['.strtoupper($row[$custom['label']]).'] '.strtoupper($row[$custom['name']]).'</option>';
				}	
